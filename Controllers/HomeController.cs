...
namespace CodeGenerator.Controllers
{
    public class HomeController : Controller
    {
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult GetTreeDBList()
        {
            ...
            _sqliteFreeSql.Dispose();
            list_ztree =  GetTreeList(list_sqlconnect);
            reponse.code = "200";
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult GetDbTable(string id,string dbName)
        {
            PageResponse reponse = new PageResponse();
            var sqlconnect = _sqliteFreeSql.Select<SqlConnect>().Where(p => p.Id == id).ToOne();
        ...
        /// <returns></returns>
        public JsonResult GetServerDbList(SqlConnect sqlconnect)
        {
            sqlconnect.DbName = "";
        ...
        /// <param name="sqlconnect"></param>
        /// <returns></returns>
        public JsonResult AddServer(SqlConnect sqlconnect)
        {
            PageResponse reponse = new PageResponse();
        ...
        /// <param name="sqlconnect"></param>
        /// <returns></returns>
        public JsonResult UpdateServer(SqlConnect sqlconnect)
        {
            PageResponse reponse = new PageResponse();
        ...
        public async Task<JsonResult> Generator(string[] tables, string[] temps)
        {
            ...
            try
            {
                ...
                foreach (var item in tables)
                {
                    ...
                    {
                        if(!string.IsNullOrEmpty(temp.TableFilter))
                        {
                            talbe.TableName = talbe.TableName.Replace(temp.TableFilter,"");
                        }
                        ...
                        if(string.IsNullOrEmpty(temp.NameSapce))
                        {
                            talbe.NameSapce = talbe.TableName;
                        }
                        else
                        {
                            talbe.NameSapce = temp.Name;
                        }
                        ...
                        var url = temp.FilePath;
                         WriteViewAsync(url, name, result);
                    }
                }
                 WriteViewAsync("temp/", "dbset.cs", dbset);
                 WriteViewAsync("temp/", "automap.cs", automap);
                reponse.code = "200";
                reponse.status = 0;
                reponse.msg = "生成成功!";
            }
        ...
        /// <returns></returns>
        private void WriteViewAsync(string url, string name, string html)
        {
            //获取根目录
        ...
        private List<zTree> GetTreeList(List<SqlConnect> list_SqlConnect)
        {
            List<zTree> list_ztree = new List<zTree>();
            //根节点
